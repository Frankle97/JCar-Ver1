...
class CalendarView extends CalendarController {
	JFrame frame;
	JButton[] button;
	JButton[] ccc;
	JLabel label;
			...
			ccc[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (e.getSource().equals(ccc[a])) {
						...
						frame.setVisible(false);
						MycarController.users.get(MycarController.cnt).setReservYear(Integer.toString(year));
						MycarController.users.get(MycarController.cnt).setReservMonth(Integer.toString(month));
						MycarController.users.get(MycarController.cnt).setReservDate(ccc[a].getText());
						chk = true;
						new ReservationView().selectTime();
					}
