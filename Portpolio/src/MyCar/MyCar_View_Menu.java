...
class MyCar_View_Menu extends MyCar_Controller {
	JFrame frame;
	JButton[] button;
	JLabel[] label;
	ImageIcon[] icon;
				...
				public void actionPerformed(ActionEvent e) {
					if (e.getSource().equals(button[0])) {
						if (users.get(cnt).getPrice() != 0) {
							...
							new MyCar_View_ChoiceModel().choiceModel();
							frame.dispose();
						}
						...
						new MyCar_View_TodayIssue().showIssue();
					}
